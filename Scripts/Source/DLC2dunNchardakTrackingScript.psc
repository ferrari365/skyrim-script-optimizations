...
;Internal variables.
int registryIndex = -1		;Last filled registry slot. ferrari365 - not used anymore
...
;/
ferrari365 - this script has been heavily reworked to fix a bug where the fixed size Registry and NameClearingAliases arrays
that control Nchardak's submersible objects can get filled with objects outside of the dungeon, causing them to permamently
eat up the available indexes, overflow the arrays and prevent any future items from getting added to them. This bug manifests itself
as the player being unable to interact with control pedestals after lowering the water level until leaving and reentering the room.
The registration system has been reworked to dynamically add/remove items from the arrays as the player goes through the dungeon and
objects get loaded/unloaded on each transition (see also complementary changes to DLC2dunNchardakSubmersible and DLC2dunNchardakDoorSeal).
/;
...
;ferrari365 - If the object is already in the registry, return its existing index. Otherwise find the first empty index in the registry and put the object in it.
int Function IndexRequest(ObjectReference obj)
	int indexInArray = Registry.Find(obj)
	if (indexInArray > -1)
		;If the object is already in the index, don't add it again. Just return its location.
		return indexInArray
	Else
		indexInArray = Registry.Find(None)
		if (indexInArray >= NameClearingAliases.Length)
; 			Debug.Trace("ERROR: Nchardak Tracking received an index request, but is out of indices!")
		Else
			Registry[indexInArray] = obj
			return indexInArray
		EndIf
	EndIf
EndFunction
...
;/
ferrari365 - original registration logic
...
;ferrari365 - not used anymore, leaving it in a compatible state in case it's needed for external calls
